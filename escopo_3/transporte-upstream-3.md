# **Upstream 3 (Transporte e Distribuição - Cargas Específicas) – Escopo 3**

Calculadora para emissões de GEE referentes ao transporte e distribuição a montante de cargas com características específicas (refrigeradas, perigosas, a granel), quando o fator de emissão depende do tipo de carga.

---

## **Endpoints de Cálculo**

### **1. POST /api/v1/upstream-3**

Calcula emissões de GEE para uma operação de upstream 3 **SEM persistir no banco**.
...
#### **Parâmetros Obrigatórios (conforme `Upstream3Input`):**
...
#### **Exemplo de Uso:**
```bash
curl -X POST "http://localhost:8080/api/v1/upstream-3" \
...
#### **Resposta de Sucesso (exemplo com campos retornados pelo `Upstream3Output`):**
```json
{
  "status": "success",
  "data": {
...
### **2. POST /api/upstream-3**

Calcula e persiste emissões de GEE para upstream 3 **com persistência no banco**.

#### **Requisição:** (parâmetros de persistência idênticos aos dos outros upstream)
```json
{
  "id_veiculo": 21,
...
#### **Resposta de Sucesso:** Similar ao `POST /api/upstream-3/calcular`, com `ids_gerados` e `metadados` de persistência.

---

## **Endpoints de Consulta**

- `GET /api/upstream-3/unidade` — consulta por unidade
- `GET /api/upstream-3/{documento_id}/detalhes` — detalhes do documento
- `GET /api/upstream-3/veiculos` — lista de veículos relevantes
