# **Upstream 4 (Transporte e Distribuição - Última Milha) – Escopo 3**
...
Calcula emissões de GEE para uma operação de upstream 4 **SEM persistir no banco**.
...
#### **Exemplo de Uso:**
```bash
curl -X POST "http://localhost:8080/upstream-4" \
...
### **2. POST /api/upstream-4**

Calcula e persiste emissões de GEE para upstream 4 **com persistência no banco**. Parâmetros de persistência seguem o padrão dos outros endpoints upstream.

---

## **Endpoints de Consulta**

- `GET /api/upstream-4/unidade`
- `GET /api/upstream-4/{documento_id}/detalhes`
- `GET /api/upstream-4/veiculos`
